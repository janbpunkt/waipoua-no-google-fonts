...
if ( ! class_exists( 'WC_AM_Client_25' ) ) {
	class WC_AM_Client_25 {
		...
		public $default_product_id = ''; // To check for updates without activation
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '', $default_product_id ) {
			...
			$this->default_product_id = absint( $default_product_id );
			...
			if ( is_admin() ) {
				...
				// Add JS for admin notice dismissability
				add_action('admin_enqueue_scripts', array( $this, 'admin_notice_dismissability' ) );
				...
				$this->wc_am_settings_menu_title         = $this->software_title . esc_html__( ' Activation', esc_attr( $text_domain ) );
				$this->wc_am_settings_title              = $this->software_title . esc_html__( ' Product Key Activation', esc_attr( $text_domain ) );
				$this->wc_am_menu_tab_activation_title   = esc_html__( 'Product Key Activation', $this->text_domain );
				$this->wc_am_menu_tab_deactivation_title = esc_html__( 'Product Key Deactivation', $this->text_domain );
				...
				// We are starting with version 2, so we don't need to migrate any data.
		...
		public function register_menu() {
			add_options_page( $this->software_title . esc_html__( ' Activation', esc_attr( $this->text_domain ) ), $this->software_title . esc_html__( ' Activation', esc_attr( $this->text_domain ) ), 'manage_options', $this->wc_am_activation_tab_key, array(
				$this,
				'config_page'
			) );
		}
		...
		public function activation() {
			...
			// I guess we should ignore this option, at least without pre 2.0 migration
			if ( get_option( $this->wc_am_instance_key ) === false ) {
				//$api_key = array(
		...
		public function inactive_notice() { ?>
			...
			if ( ! $this->is_admin_notice_active( 'disable-inactive_notice' ) ) {
				return;
			} ?>
			<div data-dismissible="disable-inactive_notice" class="notice notice-error is-dismissible">
				<p><?php printf( __( '<strong>%s</strong> supports automatic updates using WordPress\' update mechanism. To use this feature you need to activate the theme using your Product Key, which can be found in your Elmastudio account dashboard. The theme stays fully functional without activation and still can be updated manually. %sClick here%s to activate <strong>%s</strong>.', $this->text_domain ), esc_attr( $this->software_title ), '<a href="' . esc_url( admin_url( 'options-general.php?page=' . $this->wc_am_activation_tab_key ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
			</div>
			<?php
		}
		...
		// Enqueue JS for admin notice dismissability
		function admin_notice_dismissability() {

			if ( is_customize_preview() ) {
				return;
			}

			wp_enqueue_script('admin_notice_dismissability', get_template_directory_uri() . '/assets/js/admin_notice_dismissability.js', array( 'jquery', 'common' ), false,	true);
			wp_localize_script(
				'admin_notice_dismissability',
				'dismissible_notice',
				array(
		...
		public function is_admin_notice_active( $arg ) {
			$db_record   = $this->get_admin_notice_cache( $arg );
		...
		public function get_admin_notice_cache( $id = false ) {
			...
			$cache_key = $this->text_domain . '-' . md5( $id );
		...
		public function dismiss_admin_notice() {
			...
			$cache_key = $this->text_domain . '-' . md5( $option_name );
			update_site_option( $cache_key, 'forever' );
			wp_die();
		}
		...
		public function config_page() {
			...
			?>
            <div class='wrap'>
                <h2><?php echo $this->software_title . esc_html__( ' Product Key Activation', esc_attr( $this->text_domain ) ); ?></h2>
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->wc_am_api_key_key, esc_html__( 'Product Key Activation', $this->text_domain ), array(
				$this,
				'wc_am_api_key_text'
			), $this->wc_am_activation_tab_key );
			add_settings_field( $this->wc_am_api_key_key, esc_html__( 'Product Key', $this->text_domain ), array(
				$this,
				'wc_am_api_key_field'
			...
			add_settings_field( 'status', esc_html__( 'Product Key Status', $this->text_domain ), array(
				$this,
			...
			add_settings_section( 'deactivate_button', esc_html__( 'Product Key Deactivation', $this->text_domain ), array(
				$this,
				'wc_am_deactivate_text'
			), $this->wc_am_deactivation_tab_key );
			add_settings_field( 'deactivate_button', esc_html__( 'Deactivate Product Key', $this->text_domain ), array(
				$this,
			...
			if ( $_REQUEST[ 'option_page' ] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results == false && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error', esc_html__( 'Connection failed to the Elmastudio server. Try again later. There may be a problem on your server preventing outgoing requests.', $this->text_domain ), 'error' );
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->data[ $this->wc_am_api_key_key ] != '' ) {
				...
				if ( $activate_results[ 'success' ] === true && $activate_results[ 'deactivated' ] === true ) {
					...
					if ( ! empty( $this->wc_am_activated_key ) ) {
						...
						update_option( $this->wc_am_activated_key, 'Deactivated' );
						add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', esc_html__( 'Product Key deactivated.', $this->text_domain ) . esc_attr( "{$activate_results['activations_remaining']}." ), 'updated' );
					}
		...
		public function replace_license_key( $current_api_key ) {
			...
			add_settings_error( 'not_deactivated_text', 'not_deactivated_error', esc_html__( 'The Product Key could not be deactivated. Use the Product Key Deactivation tab to manually deactivate the Product Key before activating a new Product Key.', $this->text_domain ), 'updated' );
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php esc_html_e( 'Deactivates an Product Key so it can be used on another blog.', $this->text_domain ); ?></span>
			<?php
		}
		...
		public function update_check( $transient ) {
			...
			// Set default product ID if no product ID was given
			if ( '' == $this->product_id ) {
				$args['product_id'] = $this->default_product_id;
			}
